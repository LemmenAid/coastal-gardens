...
- author (ForeignKey linked to User model)
...
- is_member_story (BooleanField)

Relationships:
- One-to-Many with User (author)
- Many-to-Many with User Profile (saved_posts)

Purpose: 
Represents blog posts and member stories.

#### 4. Comment Model

Attributes:
- post (ForeignKey linked to Post model)
- author (ForeignKey linked to User model)
- body (TextField)
...
Relationships:
- One-to-Many with Post (post)
- One-to-Many with User (author)

Purpose: 
Handles comments on blog posts and member stories.
...
-- Comments linked to posts and authored by users.
- Many-to-Many: Users save multiple posts through the User Profile model.

<br>
This schema efficiently organizes data to support Coastal Gardens' functionality, such as user profiles, blog posts, member stories, comments, and contact forms. It leverages Django's ORM to enforce relationships and constraints.
